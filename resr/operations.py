...
def getContracts(client: AlgodClient) -> Tuple[bytes, bytes]:
    """Get the compiled TEAL contracts for the resr.
...
def createApp(
    ...
    Args:
        ...
        sender: The account that will create the resr application.
        startTime: A UNIX timestamp representing the start time of the resr.
            This must be greater than the current UNIX timestamp.
        endTime: A UNIX timestamp representing the end time of the resr. This
            must be greater than startTime.
        reserve: The reserve amount of the resr. If the resr ends without
            a bid that is equal to or greater than this amount, the resr will
    ...
    Returns:
        The ID of the newly created resr app.
    """
...
def setupApp(
    ...
    """Finish setting up an resr.

    This operation funds the app resr escrow account, opts that account into
    the NFT, and sends the NFT to the escrow account, all in one atomic
    transaction group. The resr must not have started yet.
...
def closeTransaction(client: AlgodClient, appID: int, closer: Account):
    """Close an resr.

    This action can only happen before an resr has begun, in which case it is
    cancelled, or after an resr has ended.

    If called after the resr has ended and the resr was successful, the
    NFT is transferred to the winning bidder and the resr proceeds are
    transferred to the seller. If the resr was not successful, the NFT and
    ...
    Args:
        ...
        closer: The account initiating the close transaction. This must be
            either the seller or resr creator if you wish to close the
            resr before it starts. Otherwise, this can be any account.
    """
    appGlobalState = getAppGlobalState(client, appID)

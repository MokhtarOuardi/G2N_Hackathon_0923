...
def approval_program():
    ...
    on_create = Seq(
        ...
        Assert(
            And(
                ...
                # TODO: should we impose a maximum resr length?
            )
        ),
        Approve(),
    )
    ...
    on_delete = Seq(
        If(Global.latest_timestamp() < App.globalGet(start_time_key)).Then(
            Seq(
                # the resr has not yet started, it's ok to delete
                Assert(
                    Or(
                        # sender must either be the seller or the resr creator
                        Txn.sender() == App.globalGet(seller_key),
                        Txn.sender() == Global.creator_address(),
                    )
                ),
                # if the resr contract account has opted into the nft, close it out
                closeNFTTo(App.globalGet(nft_id_key), App.globalGet(seller_key)),
                # if the resr contract still has funds, send them all to the seller
        ...
        If(App.globalGet(end_time_key) <= Global.latest_timestamp()).Then(
            Seq(
                # the resr has ended, pay out assets
                ...
                .Then(
                    ...
                    .Then(
                        # the resr was successful: send lead bid account the nft
                        closeNFTTo(
                    ...
                    .Else(
                        Seq(
                            # the resr was not successful because the reserve was not met: return
                ...
                .Else(
                    # the resr was not successful because no bids were placed: return the nft to the seller
                    closeNFTTo(App.globalGet(nft_id_key), App.globalGet(seller_key))
                ),
...
if __name__ == "__main__":
    with open("transaction_approval.teal", "w") as f:
        compiled = compileTeal(approval_program(), mode=Mode.Application, version=5)
        f.write(compiled)

    with open("transaction_clear_state.teal", "w") as f:
